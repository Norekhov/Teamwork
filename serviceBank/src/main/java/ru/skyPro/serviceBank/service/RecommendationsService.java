...
public class RecommendationsService {
    ...
    public ClientRecommendation getClientRecommendation(UUID user) {
        ...
        if (listProducts.contains(RECOMMENDATION_ONE)) {
            bankServiceRecommendations.add(new BankRecommendation("147f6a0f-3b91-413b-ab99-87f081d60d5a", RECOMMENDATION_ONE, DESCRIPTION_ONE));
            client = new ClientRecommendation(user, bankServiceRecommendations);
            return client;
        } else if (listProducts.contains(RECOMMENDATION_TWO)) {
            bankServiceRecommendations.add(new BankRecommendation("59efc529-2fff-41af-baff-90ccd7402925", RECOMMENDATION_TWO, DESCRIPTION_TWO));
            client = new ClientRecommendation(user, bankServiceRecommendations);
            return client;
        } else if (listProducts.contains(RECOMMENDATION_THREE)) {
            bankServiceRecommendations.add(new BankRecommendation("ab138afb-f3ba-4a93-b74f-0fcee86d447f", RECOMMENDATION_THREE, DESCRIPTION_THREE));
            client = new ClientRecommendation(user, bankServiceRecommendations);
            return client;
        } else {
